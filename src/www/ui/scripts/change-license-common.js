...
function scheduleBulkScanCommon(resultEntity, callbackSuccess) {
  ...
  for (i = 0; i < bulkActions.length; i++) {
    bulkActions[i]["reportinfo"] = $("#"+bulkActions[i].licenseId+"reportinfoBulk").attr('title').trim();
    bulkActions[i]["acknowledgement"] = $("#"+bulkActions[i].licenseId+"acknowledgementBulk").attr('title').trim();
    bulkActions[i]["comment"] = $("#"+bulkActions[i].licenseId+"commentBulk").attr('title').trim();
  }
...
function submitTextModal(){
  if(whatType == 0) {
    ...
    clearingBulkVal = $("#"+ whatLicId + whatCol + whatType).attr('title').trim();
    if(clearingBulkVal !== null && clearingBulkVal !== ''){
      $("#"+ whatLicId + whatCol + whatType).html(clearingBulkVal.slice(0, 10) + "...");
    }
  }
}
